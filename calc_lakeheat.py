...
def calc_lakeheat(models,forcings,future_experiment, indir_lakedata, years_grand, resolution,outdir, years_isimip,start_year, end_year, flag_scenario, flag_savelakeheat, rho_liq, cp_liq, rho_ice, cp_ice):
    ...
    for model in models:
        ...
        depth_per_layer,temp = calc_depth_per_layer(flag_scenario, indir_lakedata, years_grand, start_year,end_year, resolution, model,outdir)
...
def calc_lakeheat_with_volume(models,forcings,future_experiment, indir_lakedata, years_grand, resolution,outdir, years_isimip,start_year, end_year, flag_scenario, flag_savelakeheat, flag_volume, rho_liq, cp_liq, rho_ice, cp_ice):
## rewrite of lakeheat calculation formula with lake area taken into immeadiate lake layer calculation. 
    # define experiment
    experiment= 'historical_'+future_experiment # can also be set to 'historical' or 'rcp60', but start_year will in this case have to be within year range
    ...
    for model in models:
        ...
        # calculate depth per layer 
        volume_per_layer = calc_volume_per_layer(flag_scenario, resolution, indir_lakedata, years_grand, start_year,end_year, model,outdir, flag_volume)
        ...
        for forcing in forcings:
            ...
            if os.path.isfile(outdir_model['watertemp']+outfile_annual['watertemp']): 
               ...
               # icefrac = ds_icefrac.lakeicefrac.values
                ...
                #lakeheat_layered =  ((rho_liq * cp_liq * (1-icefrac)) + (rho_ice * cp_ice * icefrac) )* depth_per_layer* laketemp
                lakeheat_layered =  rho_liq * cp_liq * volume_per_layer * laketemp

                # create a 2D mask to only give 0 to grid cells where there are or will be lakes (saving memory) 
                mask_nan = np.isnan(lakeheat_layered[-1,:,:,:]).sum(axis=0)!=np.shape(np.isnan(lakeheat_layered[-1,:,:,:]))[0]
                # sum up for total layer (less memory)
                lakeheat_forcing = np.empty([lakeheat_layered.shape[0],lakeheat_layered.shape[2],lakeheat_layered.shape[3]])
                for i in range(lakeheat_layered.shape[2]):
                    for j in range(lakeheat_layered.shape[3]):
                        if mask_nan[i,j]: 
            ...
            if forcing == 'miroc5' and model == 'GOTM':
                ...
                # clean up
                del laketemp, ds_laketemp, lakeheat_layered
